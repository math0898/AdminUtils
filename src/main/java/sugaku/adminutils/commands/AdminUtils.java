...
public class AdminUtils extends AbstractCommand implements CommandExecutor {
    ...
    private void updateSubcommand(CommandSender sender) {
        Bukkit.getScheduler().runTaskLater(main.plugin, () -> Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "restart"), 1400);
        Bukkit.getScheduler().runTaskLater(main.plugin, () -> Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "kickall &8[&cAdminUtils&8] &7The server is updating. See you in a few!"), 1200);
        Bukkit.getScheduler().runTaskLater(main.plugin, () -> Bukkit.setWhitelist(false), 1399);
        ...
        for (Player p : players) {
            ...
            Bukkit.getScheduler().runTaskLater(main.plugin, () -> AbstractCommand.send(p, "The server will restart in 30 seconds!"), 600);
            Bukkit.getScheduler().runTaskLater(main.plugin, () -> AbstractCommand.send(p, "The server will restart in 15 seconds!"), 900);
            Bukkit.getScheduler().runTaskLater(main.plugin, () -> AbstractCommand.send(p, "The server will restart in 5 seconds!"), 1100);
            Bukkit.getScheduler().runTaskLater(main.plugin, () -> AbstractCommand.send(p, "Restarting in 4..."), 1120);
            Bukkit.getScheduler().runTaskLater(main.plugin, () -> AbstractCommand.send(p, "Restarting in 3..."), 1140);
            Bukkit.getScheduler().runTaskLater(main.plugin, () -> AbstractCommand.send(p, "Restarting in 2..."), 1160);
            Bukkit.getScheduler().runTaskLater(main.plugin, () -> AbstractCommand.send(p, "Restarting in 1..."), 1180);
            Bukkit.getScheduler().runTaskLater(main.plugin, () -> AbstractCommand.send(p, "Server is restarting. See you in a few!"), 1200);
        }
    }
